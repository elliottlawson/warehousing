...
it('can create stock', function () {
    ...
    expect($this->stock->inventory->id)->toBe($this->inventory->id);
    expect($this->stock->location->id)->toBe($this->location->id);
});

it('has a good relationship with inventory and locations', function () {
    expect($this->stock->id)->toBeIn($this->inventory->stock->pluck('id'));
    expect($this->stock->id)->toBeIn($this->location->stock->pluck('id'));
});
